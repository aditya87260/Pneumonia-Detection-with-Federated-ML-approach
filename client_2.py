...
class PneumoniaClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        callback_lr = ReduceLROnPlateau(monitor='val_loss', patience = 3, cooldown=0, verbose=1, factor=0.6, min_lr=0.000001)
        r = self.model.fit(self.train_data, epochs=1, validation_data=self.val_data, class_weight={0:7, 1:0.5}, callbacks=[callback_lr])
        hist = r.history
...
from flwr.client.supernode import SuperNodeClient
...
if __name__ == "__main__":
    ...
    SuperNodeClient.connect("127.0.0.1:8080", client)
